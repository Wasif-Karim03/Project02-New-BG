...
/**
 * Start a Stripe Checkout for a one-time gift. The donor's chosen dollar amount is
 * converted to minor units and validated (USD) server-side. Requires a real
 * STRIPE_SECRET_KEY (test or live); without one, Stripe rejects and we surface an error.
 */
export async function startCheckoutAction(formData: FormData) {
  ...
  try {
    ...
    const session = await createCheckoutSession(parsed.data, {
      successUrl: `${origin}/donate/thank-you?ref={CHECKOUT_SESSION_ID}`,
