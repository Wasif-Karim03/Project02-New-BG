...
export default async function ApplyStartPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return (
    <main className="mx-auto w-full max-w-lg px-6 py-16">
      ...
      <h1 className="mt-2 text-3xl font-bold tracking-tight text-ink">Apply for a scholarship</h1>
      <p className="mt-3 text-sm leading-relaxed text-ink-2">Create an applicant account (or continue an existing application), fill the form, and verify your email. An admin reviews every application. There is no application fee.</p>
      ...
      <form action={createAccountAction} className="mt-8 rounded-2xl border border-hairline bg-ground-2 p-6 shadow-sm">
