...
export default async function DonorVerifyPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return (
    <main className="mx-auto max-w-md px-6 py-16">
      ...
      <p className="mt-2 text-sm text-black/60">We emailed a 6-digit code. Enter it to activate your donor account. (In dev, the code prints to the server console.)</p>
      ...
      <form action={verifyDonorAction} className="mt-6 flex items-end gap-3">
      ...
      <form action={resendDonorAction} className="mt-4"><button type="submit" className="text-sm text-blue-700 underline">Resend code</button></form>
    </main>
  );
}
