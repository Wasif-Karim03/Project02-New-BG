...
export default async function MentorApplyVerifyPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return (
    <main className="mx-auto w-full max-w-md px-6 py-16">
      ...
      <p className="mt-3 text-sm leading-relaxed text-ink-2">We emailed a 6-digit code. Enter it to finish submitting your mentor application. (In dev, the code prints to the server console.)</p>
      {dev && <div className="mt-6 rounded-xl border border-accent-3/50 bg-accent-3/15 px-4 py-3 text-sm text-ink">Dev mode: your code is <strong className="tracking-widest">{dev}</strong>.</div>}
      ...
      <form action={verifyMentorCodeAction} className="mt-6 flex items-end gap-3 rounded-2xl border border-hairline bg-ground-2 p-6 shadow-sm">
